...
def index_to_action(index: int, game_state: GameState) -> Action:
    ...
    if 0 <= index <= 9:
        ...
        return Action("take_tokens", tokens_taken=tokens_taken)
        ...
        if card_index < len(visible_cards):
            ...
            for t, cards in game_state.board.items():
                if card in cards:
                    tier = t
                    break
            return Action("reserve", target=card, tier=tier)
        else:
...
def mask_illegal_actions(q_values, legal_action_indices):
    mask = torch.full_like(q_values, float('-inf'))  # Start with -inf everywhere
