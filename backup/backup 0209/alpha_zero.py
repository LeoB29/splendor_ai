...
def self_play_episode(model: PolicyValueNet, mcts_simulations: int = 100, device: str = "cpu", temperature: float = 1.0,
                      ...
                      add_dirichlet: bool = True) -> Tuple[List[Sample], int]:
...
def train_on_batch(model: PolicyValueNet, optimizer: optim.Optimizer, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], device: str = "cpu",
                   policy_weight: float = 1.0, value_weight: float = 1.0, grad_clip: Optional[float] = None) -> Dict[str, float]:
    X, P, Z = batch
