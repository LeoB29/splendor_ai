    ...
    def run(self, root_state: GameState, temperature: float = 1.0) -> Tuple[np.ndarray, int]:
        ...
        if total > 0:
            if temperature != 1.0:
                ...
                pi = pi / total
        return pi, int(np.argmax(pi) if temperature == 0 else np.random.choice(len(pi), p=(pi / pi.sum()) if pi.sum() > 0 else None))
...
def self_play_episode(model: PolicyValueNet, mcts_simulations: int = 100, device: str = "cpu", temperature: float = 1.0) -> Tuple[List[Sample], int]:
    ...
    while not state.is_terminal:
        ...
        # Sample an action from pi (with temperature already applied inside run)
        if pi.sum() == 0:
            ...
            a_idx = np.random.choice(len(pi), p=pi / pi.sum())
